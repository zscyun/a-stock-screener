...
# ── 选股筛选 v1.0.4 — 涨停风险检测 + 可买入性因子 (v1.0.3+) ──
# 基本用法
...
## 🆕 v1.0.5 Changelog (2026-05-12)

### 核心升级
- **组合建议联动 chase_info**: 保守型现在只选🟢可追标的，🔴别碰自动排除；进取型优先排🟢、降权🔴
- **_is_toxic 门槛收紧 (v1.0.5+)**:
  - ROE 门槛从 2%→3%
  - PE>500 + ROE<5% → 壳股也排除
  - 毛利率门槛从 5%→8%
- **策略文案差异化**: 🟢→"突破买入/分批建仓", 🟡→"等RSI回调至75以下","连板期等分歧低吸", 🔴→"情绪高潮坚决回避","技术破位建议换标的"
- **.bak 残留文件清理**, git index 瘦身

### Bug Fixes
- 保守型池子可能包含🔴高风险标的 → v1.0.5 修复
- 进取型未区分 chase 风险等级进行排序 → v1.0.5 新增 `_chase_sort_key`

---

## 🆕 v1.0.4 Changelog (2026-05-12)

### 核心升级：解决"涨停股买不到"问题 + 安全追涨模式
| # | 功能 | 说明 |
|---|------|------|
| 1 | `_score_limit_up_risk` | 涨停风险因子 — 检测连续涨停≥3天/接近涨停，严重扣分 |
| 2 | `_score_buyability` | 可买入性因子 — MA5偏离度+RSI超买检查，评估建仓空间 |
| 3 | `--chase-mode` (🆕) | **安全追涨模式** — 综合5维度标注🟢🟡🔴风险等级+上车策略 |

### 安全追涨模式 (`--chase-mode`) 详解
```bash
python3 stock_screen.py screen --limit 15 --chase-mode
```
- **5维评估**: 连续涨停次数 + RSI超买程度 + 成交量健康度 + MACD状态 + K线形态
- **🟢可追**: 首次/单板启动,量能健康 → 突破买入/回踩MA5介入
- **🟡谨慎**: 连板或RSI>80 → 等回调至75以下再介入
- **🔴别碰**: ≥3板+极端情绪(RSI>90) → 高潮期,建议换标的
- **输出增强**: Top列表新增"追涨/策略"列; 深度分析卡新增安全追涨评估详情

### 打分系统变化
- **趋势因子组** 从 4个 → 6个（新增 limit_up_risk=1.5, buyability=1.2）
- **连续涨停≥3天** → score=0.1（严重追高风险，大概率买不到）
- **单日涨幅>9%** → score=0.4（接近涨停，建仓空间小）
- **MA5偏离≥8% + RSI>80** → 可买入性扣 0.7分

### Bug Fix & 清理 (v1.0.3)
- `tushare_compat.py`: f-string 内 `\n` 转义符在 Python<3.12 报错 → 提取为独立函数修复
- 注释中的 TODO #2/#3 标记全部清理，避免新用户困惑
- Staleness 自动刷新 — screen_hot_pool 超24h静默触发 discover_stocks()
