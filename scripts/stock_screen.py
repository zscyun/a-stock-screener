...
# ─── v5.0: 池子健康度管理阈值 ───
POOL_MAX_AGE_HOURS = 24                  # 池子最大存活时间(小时),超过需强制刷新
HEALTH_DECAY_PER_DAY = 0.5               # 每日活动分自然衰减
HEALTH_STALE_THRESHOLD_DAYS = 3          # 超过N天未发现→标记stale
HEALTH_CONTINUOUS_LOSE_THRESHOLD = 5     # 连续下跌N天→绩效预警
...
def _check_pool_staleness(pool_state: Dict) -> Dict[str, Any]:
    """检查池子是否过时,返回健康度报告"""
    report = {'status': 'healthy', 'age_hours': 0, 'issues': []}

    last_scan = pool_state.get('last_scan')
    if not last_scan:
        report['status'] = 'stale'
        report['issues'].append('池子从未扫描过,需要完整发现流程')
        return report
    ...
    try:
        ...
        report['age_hours'] = round(age_hours, 1)

        if age_hours > POOL_MAX_AGE_HOURS:
            report['status'] = 'stale'
            report['issues'].append(f'池子已 {age_hours:.0f}h 未刷新(阈值={POOL_MAX_AGE_HOURS}h)')
    except Exception as e:
...
def _check_stock_performance(code: str) -> Dict[str, Any]:
    """检查个股近期绩效:连续下跌天数 + 近1月收益"""
    result = {'continuous_loss_days': 0, 'monthly_return': None, 'flag': ''}

    try:
        kline_data = get_kline_data(code)
        if not kline_data or len(kline_data) < 20:
            return result

        closes = list(kline_data['close'])[-60:]  # 最近60天
        ...
        for i in range(len(closes)-1, -1, -1):
            if i > 0 and closes[i] < closes[i-1]:
                ...
                break
        result['continuous_loss_days'] = loss_days

        # 计算近1月收益(约20个交易日)
        if len(closes) >= 20:
            monthly_return = (closes[-1] - closes[-20]) / closes[-20] * 100
            result['monthly_return'] = round(monthly_return, 2)

        # 标记问题股
        if loss_days >= HEALTH_CONTINUOUS_LOSE_THRESHOLD:
            result['flag'] = '📉连续下跌'
    except Exception as e:
        pass
...
def _apply_activity_decay(pool_state: Dict):
    """根据天数衰减活动分数,并标记stale股票"""
    pool = pool_state.get('pool', {})
    last_scan = pool_state.get('last_scan')
    if not last_scan:
        return
    ...
    try:
        days_since_scan = (datetime.now() - datetime.fromisoformat(last_scan)).days
        decay = HEALTH_DECAY_PER_DAY * max(0, days_since_scan)

        for code in list(pool.keys()):
            old_act = pool[code].get('activity', ACTIVITY_NEW_ENTRY)
            new_act = max(ACTIVITY_REMOVE_THRESHOLD, round(old_act - decay, 1))
            if new_act != old_act:
                print(f"   ⏳ {code}({pool[code]['name']}): activity衰减 {old_act}→{new_act}", file=sys.stderr)
            pool[code]['activity'] = new_act

            # 标记stale(超过阈值天未发现)
            if days_since_scan >= HEALTH_STALE_THRESHOLD_DAYS:
                last_seen = pool[code].get('last_seen_days') or days_since_scan
                if last_seen >= HEALTH_STALE_THRESHOLD_DAYS:
                    print(f"   ⚠️ {code}({pool[code]['name']}): stale {last_seen}天", file=sys.stderr)
    except Exception as e:
        print(f"   ⚠️ 衰减计算失败: {e}", file=sys.stderr)


def run_pool_maintain(force_refresh=False):
    """
    v5.0: 池子健康度维护命令
    ...
    staleness = _check_pool_staleness(state)
    emoji = {'healthy': '✅', 'stale': '⚠️', 'warning': '🔶'}
    print(f"\n[Step 1] 新鲜度检查: {staleness['status']} ({staleness.get('age_hours', 'N/A')}h)", file=sys.stderr)
    for issue in staleness['issues']:
        print(f"   ❌ {issue}", file=sys.stderr)

    # Step 2 & 3: Activity decay
    _apply_activity_decay(state)
    print(f"\n[Step 2] 活动分衰减完成", file=sys.stderr)

    # Step 4: Per-stock performance check (仅对top活跃股做,避免全量耗时)
    top_stocks = sorted(pool.items(), key=lambda x: x[1].get('activity', 0), reverse=True)[:30]
    print(f"\n[Step 3] 绩效体检 ({len(top_stocks)}只Top活跃股)...")
    for code, info in top_stocks:
        perf = _check_stock_performance(code)
        if perf['flag']:
            flagged.append((code, info.get('name', ''), perf))
            print(f"   {perf['flag']} {info.get('name','')}({code}): 连跌{perf['continuous_loss_days']}天, 月收益{perf['monthly_return']}%", file=sys.stderr)
        else:
            healthy.append((code, info.get('name', ''), perf))

    # Step 5: Clean up zero-activity + flagged stocks
    print(f"\n[Step 4] 清理低活+问题股...", file=sys.stderr)
    to_remove = [k for k, v in pool.items() if v.get('activity', 0) <= ACTIVITY_REMOVE_THRESHOLD]
    for code in to_remove:
        removed.append(code)
        del pool[code]
        print(f"   🗑️ 移除: {code}({pool.get(code, {}).get('name','') or '未知'})", file=sys.stderr)

    # Step 6: Report
    final_count = len(pool)
    print(f"\n{'='*50}", file=sys.stderr)
    print(f"📊 健康报告:", file=sys.stderr)
    print(f"   初始数量: {initial_count} 只", file=sys.stderr)
    print(f"   移除数量: {len(removed)} 只", file=sys.stderr)
    print(f"   问题标记: {len(flagged)} 只", file=sys.stderr)
    print(f"   健康数量: {len(healthy)} 只", file=sys.stderr)
    print(f"   最终池子: {final_count} 只", file=sys.stderr)

    # Save updated state
    state['pool'] = pool
    _save_pool_state(state)

    # Optional: force refresh if stale
    if staleness['status'] == 'stale' and force_refresh:
        print(f"\n🔄 池子过时,触发完整发现流程...", file=sys.stderr)
        discover_stocks()
        new_state = _load_pool_state()
        print(f"   ✅ 刷新完成: {len(new_state.get('pool',{}))}只", file=sys.stderr)
    elif staleness['status'] == 'stale' and not force_refresh:
        print(f"\n💡 池子已过时,建议运行 --force-refresh 完整更新", file=sys.stderr)
...
def analyze_codes(codes_str: str, limit: int = 10,
                  ...
                  use_exa: bool = False,
                  ratio: Optional[str] = None) -> List[Dict[str, Any]]:
    """分析指定股票列表

    Args:
        use_exa: 是否启用Exa板块热度搜索 (默认False保性能)
        ratio: 价值:趋势权重比,如"3:7"(趋势优先)
    """
...
def _apply_ratio(weights: Dict, ratio_str: str) -> Dict:
    """
    根据 ratio 字符串动态调整价值/趋势因子的权重比例。

    Args:
        weights: 原始权重的副本
        ratio_str: "VALUE:TREND" 格式,如 "3:7"、"50:50"、"1:2"
                  默认 None → 保持 DEFAULT_WEIGHTS 原比例 (~63% value / ~37% trend)
    ...
    try:
        ...
        if len(parts) != 2:
            print(f"⚠️ ratio 格式错误: {ratio_str},应为 'VALUE:TREND'(如 '3:7')", file=sys.stderr)
            return weights

        v_part = float(parts[0])
        t_part = float(parts[1])
        total = v_part + t_part
        if total <= 0:
            print(f"⚠️ ratio 值必须为正数", file=sys.stderr)
            return weights

        v_ratio = v_part / total
        t_ratio = t_part / total

        # 计算当前价值/趋势因子的总权重
        current_value_total = sum(weights.get(k, 0) for k in VALUE_FACTOR_KEYS if k in weights)
        current_trend_total = sum(weights.get(k, 0) for k in TREND_FACTOR_KEYS if k in weights)

        # 目标总权重(保持总和不变)
        target_total = current_value_total + current_trend_total
        target_value_total = target_total * v_ratio
        target_trend_total = target_total * t_ratio

        # 按比例缩放各组因子
        if current_value_total > 0:
            value_scale = target_value_total / current_value_total
            for k in VALUE_FACTOR_KEYS:
                if k in weights:
                    weights[k] = round(weights[k] * value_scale, 3)

        if current_trend_total > 0:
            trend_scale = target_trend_total / current_trend_total
            for k in TREND_FACTOR_KEYS:
                if k in weights:
                    weights[k] = round(weights[k] * trend_scale, 3)
    
        # 只打印一次ratio调整信息（用全局变量追踪）
        global _RATIO_PRINTED
        try:
            is_first = not _RATIO_PRINTED
        except NameError:
            is_first = True
        if is_first and v_ratio > 0 and t_ratio > 0:
            print(f"📊 Ratio调整: 价值{v_ratio:.0%} : 趋势{t_ratio:.0%}", file=sys.stderr)
            _RATIO_PRINTED = True
    
    except (ValueError, ZeroDivisionError) as e:
        if not RATIO_LOGGED:
            print(f"⚠️ ratio解析失败: {e}", file=sys.stderr)
            RATIO_LOGGED = True
    
    return weights


def _apply_ratio_once(weights: Dict, ratio_str: str, already_printed: list) -> Dict:
    """
    包装器：确保ratio调整信息只打印一次（per session）。
    already_printed是一个可变列表，用于跨调用追踪是否已打印过。
    """
    if not ratio_str or already_printed:
        return weights
...
RATIO_LOGGED: bool = False  # ratio调整信息只打印一次
...
def compute_score(data: Dict[str, Any], weights: Optional[Dict] = None, use_exa: bool = True, ratio: Optional[str] = None) -> float:
    """
    多因子综合打分(满分10分)
    ...
    ratio: "VALUE:TREND" 格式,动态调整价值/趋势权重比
    """
    w = dict(DEFAULT_WEIGHTS)
    if weights:
        w.update(weights)
    # 应用 ratio 调整(在 custom_weights 之后)
    w = _apply_ratio(w, ratio)
...
def main():
    ...
    maintain_p = subparsers.add_parser('maintain', help='v5.0 池子健康度维护(体检+清理+报告)')
    maintain_p.add_argument('--force-refresh', action='store_true', help='过时池子自动触发完整发现流程')
    ...
    for sub in subparsers.choices.values():
        ...
        sub.add_argument('--ratio', type=str, default=None,
                        help='价值:趋势权重比,如"3:7"(趋势优先)或"7:3"(价值优先),默认保持现状(约6成价值/3成趋势)')
    ...
    try:
        ...
        if args.action == 'analyze':
            ...
            run_pool_maintain(force_refresh=getattr(args, 'force_refresh', False))
